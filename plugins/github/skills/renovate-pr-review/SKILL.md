...
- 一時しのぎの workaround より**根本的な修正**を優先する。
...
| **MIGRATE** | 破壊的変更が我々の使用箇所に影響する | 修正必須、可能なら根本的な書き換え |
| **FOLLOW-UP** | 設定ファイル（schema バージョン等）の追随が必要 | 設定ファイルを更新 |
...
破壊的変更を受けた際、型アサーション (`as unknown as T`)、`@ts-ignore`、version pinning による回避、代替ライブラリの臨時導入などの workaround を用いない。根本的な書き換えで対応する。書き換え量が大きいと判断した場合は、その旨をユーザーに提示して方針を確認する。
